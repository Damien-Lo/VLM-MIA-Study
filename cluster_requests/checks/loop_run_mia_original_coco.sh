#!/bin/bash
#SBATCH --job-name=loop_run_mia_coco_2017_ift_check
#SBATCH --output=out_loop_run_mia_coco_2017_ift_check.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
for ((set=0; set<3; set++)); do
    ...
    python /home/clo37/priv/VLM-MIA-Study/mia.py \
        ...
        job_meta_params.description="'LLaVA Model with MEMBERS=original_coco, NONMEMBERS=sharegpt_4o_global_nonmembers_300 at std set ${set}: ${STD_SETS[$set]} for sanity check'" \
        \
        path.output_dir=/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/results/2025_11_01_check/coco_check/gn_set${set} \
        \
        target_model="llava-v1.5-7b" \
        \
        data.member_dataset='coco_original_members' \
        data.member_subset=/local/scratch/clo37/datasets/COCO_2017_train/llava_fft_splits/fft_member_subset_300.arrow \
        data.member_desc_path=/home/clo37/priv/VLM-MIA-Study/gen_descriptions/llava/coco_2017/fft_member_subset_300.json \
