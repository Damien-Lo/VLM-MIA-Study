#!/bin/bash
#SBATCH --job-name=minigpt_flickr_1_exact_hyperparam_tuning
#SBATCH --output=out_minigpt_flickr_1_exact_hyperparam_tuning.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
for ((run=0; run<NUM_OF_RUNS; run++)); do
  for ((set=1; set<2; set++)); do
    ...
    python /home/clo37/priv/VLM-MIA-Study/mia.py \
        ...
        \
        path.output_dir=/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_RESULTS/minigpt/flickr/hyperparam_tuning/flickr_pretrain_member_ratio_0.5__exact_flickr_KN/run_${run}/gn_set${set} \
        \
