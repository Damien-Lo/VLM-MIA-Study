#!/bin/bash
#SBATCH --job-name=minigpt_flickr_3_half_hyperparam_tuning
#SBATCH --output=out_minigpt_flickr_3_half_hyperparam_tuning.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
member_dataset="/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_DATASETS/flickr/flickr_member_subset.json"
...
reference_dataset='["/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_DATASETS/share_gpt/global_nonmember_fullset.json","/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_DATASETS/flickr/flickr_nonmember_subset.json"]'
...
for ((run=0; run<NUM_OF_RUNS; run++)); do
  for ((set=1; set<2; set++)); do
    ...
    python /home/clo37/priv/VLM-MIA-Study/mia.py \
        ...
        \
        path.output_dir=/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_RESULTS/minigpt/flickr/hyperparam_tuning/flickr_pretrain_member_ratio_0.5__sharegpt0.5_flickr0.5_KN/run_${run}/gn_set${set} \
        \
