#!/bin/bash
#SBATCH --job-name=minigpt_flickr_v2_evaluation
#SBATCH --output=out_minigpt_flickr_v2_evaluation.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
for ((set=0; set<4; set++)); do
    ...
    python /home/clo37/priv/VLM-MIA-Study/mia.py \
        ...
        job_meta_params.description="'Evaluation on Model: IFT Llava, dataset: 0.5 flicker members and 0.5 flicker nonmembers from flickr_pretrain_member_ratio_0.5__random_flickr_KN'" \
        job_meta_params.job_type=evaluation \
        \
        path.output_dir=/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_RESULTS/llava/flickr/evaluation/flickr_pretrain_member_ratio_0.5__random_flickr_KN_eval/gn_set${set} \
        \
        target_model="minigpt-4" \
        \
        data.dataset=/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_RESULTS/llava/flickr/hyperparam_tuning/flickr_pretrain_member_ratio_0.5__random_flickr_KN/gn_set0/datasets/target_dataset.parquet \
        \
