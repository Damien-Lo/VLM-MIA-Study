#!/bin/bash
#SBATCH --job-name=coco_v2_evaluation
...
for ((set=0; set<4; set++)); do
    ...
    python /home/clo37/priv/VLM-MIA-Study/mia.py \
        ...
        job_meta_params.description="'Evaluation on Model: IFT Llava, dataset: 0.5 coco members and 0.5 flicker nonmembers from coco_2017_IFT_member_ratio_0.5__random_flickr_KN'" \
        job_meta_params.job_type=evaluation \
        \
        path.output_dir=/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_RESULTS/llava/coco/evaluation/coco_2017_IFT_member_ratio_0.5__random_flickr_KN_eval/gn_set${set} \
        \
