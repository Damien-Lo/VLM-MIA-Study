...
for ((set=1; set<4; set++)); do
  ...
  python /home/clo37/priv/VLM-MIA-Study/mia.py \
      ...
      job_meta_params.description="'Run hyperparam tuning on target set of coco members and flicker nonmembers with same exact sample of flickr nonmembers for reference set'" \
      job_meta_params.job_type=hyperparam_tuning \
      \
      path.output_dir=/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_RESULTS/llava/coco/hyperparam_tuning/coco_2017_IFT_member_ratio_0.5__exact_flickr_KN/gn_set${set} \
      \
