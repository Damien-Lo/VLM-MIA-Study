#!/bin/bash
#SBATCH --job-name=coco_v2_hyperparam_tuning
...
member_dataset="/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_RESULTS/llava/coco/hyperparam_tuning/coco_2017_IFT_member_ratio_0.5__random_flickr_KN/gn_set0/datasets/member_target_dataset.parquet"
...
for ((set=1; set<4; set++)); do
  ...
  python /home/clo37/priv/VLM-MIA-Study/mia.py \
      ...
      job_meta_params.description="'Run hyperparam tuning on target set of coco members and flicker nonmembers with different sample of flickr nonmembers for reference set'" \
      job_meta_params.job_type=hyperparam_tuning \
      \
      path.output_dir=/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_RESULTS/llava/coco/hyperparam_tuning/coco_2017_IFT_member_ratio_0.5__random_flickr_KN/gn_set${set} \
      \
      ...
      data.reference_datasets_list=["/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_DATASETS/flickr/flickr_nonmember_subset.json"] \
