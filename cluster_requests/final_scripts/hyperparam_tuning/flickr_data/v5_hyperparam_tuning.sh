#!/bin/bash
#SBATCH --job-name=flickr_v5_hyperparam_tuning_two
#SBATCH --output=out_v5_hyperparam_tuning_two.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
member_dataset="/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_RESULTS/llava/flickr/hyperparam_tuning/aditional_flickr_pretrain_member_ratio_0.5__sharegpt_KN/run_two/gn_set0/datasets/member_target_dataset.parquet"
nonmember_dataset="/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_RESULTS/llava/flickr/hyperparam_tuning/aditional_flickr_pretrain_member_ratio_0.5__sharegpt_KN/run_two/gn_set0/datasets/non_member_target_dataset.parquet"
...
for ((set=1; set<4; set++)); do
  ...
  python /home/clo37/priv/VLM-MIA-Study/mia.py \
      ...
      job_meta_params.description="'Run hyperparam tuning on target set of flickr members and flicker nonmembers with sharegpt nonmembers for reference set'" \
      job_meta_params.job_type=hyperparam_tuning \
      \
      path.output_dir=/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_RESULTS/llava/flickr/hyperparam_tuning/aditional_flickr_pretrain_member_ratio_0.5__sharegpt_KN/run_two/gn_set${set} \
      \
