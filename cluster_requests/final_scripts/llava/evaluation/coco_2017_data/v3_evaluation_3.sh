#!/bin/bash
#SBATCH --job-name=coco_v3_evaluation_one
#SBATCH --output=out_v3_evaluation_one.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
for ((set=0; set<4; set++)); do
    ...
    python /home/clo37/priv/VLM-MIA-Study/mia.py \
        ...
        job_meta_params.description="'Evaluation on Model: IFT Llava, dataset: 0.5 coco members and 0.5 flicker nonmembers from coco_2017_IFT_member_ratio_0.5__sharegpt0.5_flickr0.5_KN'" \
        job_meta_params.job_type=evaluation \
        \
        path.output_dir=/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_RESULTS/llava/coco/evaluation/coco_2017_IFT_member_ratio_0.5__sharegpt0.5_flickr0.5_KN_eval/run_3/gn_set${set} \
        \
        target_model="llava-v1.5-7b" \
        \
        data.dataset=/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/FINAL_DATA/FINAL_RESULTS/llava/coco/hyperparam_tuning/coco_2017_IFT_member_ratio_0.5__sharegpt0.5_flickr0.5_KN/run_3/gn_set0/datasets/target_dataset.parquet \
        \
