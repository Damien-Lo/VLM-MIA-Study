#!/bin/bash
#SBATCH --job-name=loop_run_mia_flickr
#SBATCH --output=out_loop_run_mia_flickr.log
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
for ratio in "${RATIOS[@]}"; do
    ...
    for ((set=0; set<3; set++)); do
        ...
        python /home/clo37/priv/VLM-MIA-Study/mia.py \
            ...
            job_meta_params.description="'LLaVA Model with MEMBERS=flickr_mixed_set_ratio_${ratio}, NONMEMBERS=sharegpt_4o_global_nonmembers_300 at std set ${set}: ${STD_SETS[$set]}'" \
            \
            path.output_dir=/local/scratch/clo37/VLM_MIA_STUDY_Archive_Data/mixed_set_results/llava/flickr/flickr_pretrain_member_ratio_${ratio}/gn_set${set} \
            \
            target_model="llava-v1.5-7b" \
            \
            data.member_dataset='flickr_mixed' \
            data.member_subset=/local/scratch/clo37/datasets/mixed_sets/flickr/flickr_pretrain_member_ratio_${ratio}/mixed_subset_memrat_${ratio}_target.arrow \
            data.member_desc_path=/home/clo37/priv/VLM-MIA-Study/gen_descriptions/llava/mixed_sets/flickr_mixed/flickr_pretrain_member_ratio_${ratio}.json \
